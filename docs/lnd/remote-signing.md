...
```shell
signer@host:~$ lncli wallet accounts list > accounts-signer.json
```
...
```shell
user@signer:~$ lncli bakemacaroon --save_to signer.custom.macaroon \
...
```shell
user@watch-only:~$ lncli createwatchonly accounts-signer.json
