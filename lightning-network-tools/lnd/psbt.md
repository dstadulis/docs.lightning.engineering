...
See [BIP174](https://github.com/bitcoin/bips/blob/master/bip-0174.mediawiki) for a full description of the PSBT format and the different _roles_ that a participant in a PSBT can have.
...
That final transaction can now, in theory, be broadcast. But **it is very important** that you **do not** publish it manually if any of the involved outputs are used to fund a channel. See [the safety warning below](psbt.md#safety-warning) to learn the reason for this.
